...
// Uncomment line below to use the CombinedIMUFactor as opposed to the standard
// ImuFactor. #define USE_COMBINED
...
int main(int argc, char *argv[]) {
  ...
  Eigen::Matrix<double, 10, 1> initial_state =
  ...
  graph->add(
      PriorFactor<Pose3>(X(correction_count), prior_pose, pose_noise_model));
  graph->add(PriorFactor<Vector3>(V(correction_count), prior_velocity,
                                  velocity_noise_model));
  graph->add(PriorFactor<imuBias::ConstantBias>(
  ...
  Matrix33 measured_omega_cov =
      Matrix33::Identity(3, 3) * pow(gyro_noise_sigma, 2);
  Matrix33 integration_error_cov =
      Matrix33::Identity(3, 3) *
      1e-8;  // error committed in integrating position from velocities
  Matrix33 bias_acc_cov =
      Matrix33::Identity(3, 3) * pow(accel_bias_rw_sigma, 2);
  Matrix33 bias_omega_cov =
      Matrix33::Identity(3, 3) * pow(gyro_bias_rw_sigma, 2);
  ...
  // Store previous state for the imu integration and the latest predicted
  // outcome.
  NavState prev_state(prior_pose, prior_velocity);
  NavState prop_state = prev_state;
  imuBias::ConstantBias prev_bias = prior_imu_bias;

  // Keep track of the total error over the entire run for a simple performance
  // metric.
  ...
  while (file.good()) {
    ...
    if (type == 0) {  // IMU measurement
      ...
      imu_preintegrated_->integrateMeasurement(imu.head<3>(), imu.tail<3>(),
                                               dt);
      ...
      CombinedImuFactor imu_factor(X(correction_count - 1),
                                   V(correction_count - 1), X(correction_count),
                                   V(correction_count), B(correction_count - 1),
