 ...
 * class. In this example we defined operator * outside class in the end
 */
...
struct traits<gtsamexamples::Point2c> {
  ...
  // print
  static void Print(const gtsamexamples::Point2c& m,
                    const std::string& str = "") {
    std::cout << str << "(" << m.x << ", " << m.y << ")" << std::endl;
  }

  // equality with optional tol
  static bool Equals(const gtsamexamples::Point2c& m1,
                     const gtsamexamples::Point2c& m2, double tol = 1e-8) {
