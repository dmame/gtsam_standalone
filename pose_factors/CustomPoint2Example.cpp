 ...
 * The robot moves from x1 to x5 with 2 stepsize, with odometry information
 * between each pair. prior factor on first state, remainings are open-loop The
 * graph strcuture is shown:
 *
 *  p-x1 - x2 - x3 - x4 - x5
 */
...
int main(int argc, char** argv) {
  ...
  graph.add(BetweenFactor<Point2c>(Symbol('x', 1), Symbol('x', 2),
                                   Point2c(2, 0), odomModel));
  graph.add(BetweenFactor<Point2c>(Symbol('x', 2), Symbol('x', 3),
                                   Point2c(2, 0), odomModel));
  graph.add(BetweenFactor<Point2c>(Symbol('x', 3), Symbol('x', 4),
                                   Point2c(2, 0), odomModel));
  graph.add(BetweenFactor<Point2c>(Symbol('x', 4), Symbol('x', 5),
                                   Point2c(2, 0), odomModel));
