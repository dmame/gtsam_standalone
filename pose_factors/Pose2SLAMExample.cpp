...
// In planar cases we use Pose2 variables (x, y, theta) to represent the robot
// poses in SE(2)
...
// e.g. pose varibles are generally indexed as 'x' + number, and landmarks as
// 'l' + numbers
...
int main(int argc, char** argv) {
  ...
  // A prior factor consists of a mean value and a noise model (covariance
  // matrix)
  noiseModel::Diagonal::shared_ptr priorModel =
      noiseModel::Diagonal::Sigmas(Vector3(1.0, 1.0, 0.1));
  ...
  graph.add(BetweenFactor<Pose2>(Symbol('x', 1), Symbol('x', 2), Pose2(5, 0, 0),
                                 odomModel));
  graph.add(BetweenFactor<Pose2>(Symbol('x', 2), Symbol('x', 3),
                                 Pose2(5, 0, -M_PI_2), odomModel));
  graph.add(BetweenFactor<Pose2>(Symbol('x', 3), Symbol('x', 4),
                                 Pose2(5, 0, -M_PI_2), odomModel));
  graph.add(BetweenFactor<Pose2>(Symbol('x', 4), Symbol('x', 5),
                                 Pose2(5, 0, -M_PI_2), odomModel));
  ...
  graph.add(BetweenFactor<Pose2>(Symbol('x', 5), Symbol('x', 2),
                                 Pose2(5, 0, -M_PI_2), loopModel));
